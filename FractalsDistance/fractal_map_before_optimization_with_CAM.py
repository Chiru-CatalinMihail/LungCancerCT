...
crt_folder = './pacienti_cu_gradcam'
...
def calculate_local_fractal_dimensions(matrix, box_size=5, step=2):
    ...
    return result_matrix, result_matrix_max
...
if __name__ == '__main__':
    ...
    for file_path in tqdm(files):
        ...
        raw_image = data[0].numpy().squeeze(-1) * 2486 - 1024 # De ce 2486 și 1024??? ma reuit
        gt_label = data[1].numpy().squeeze(-1)
        
        fig, axes = plt.subplots(1, 6, figsize=(15, 10))
        axes[0].imshow(raw_image, cmap='gray')
        axes[1].imshow(gt_label, cmap='gray')
        axes[2].imshow(data[2].numpy().squeeze(-1), cmap='gray')
        axes[3].imshow(data[3].numpy().squeeze(-1), cmap='gray')
        axes[4].imshow(data[4].numpy().squeeze(-1), cmap='gray')
        axes[5].imshow(data[5].numpy().squeeze(-1), cmap='gray')
        plt.show()

        print(gt_label.shape)

        #  contur leziune
        threshold = threshold_otsu(gt_label)
        contour = extract_contour(gt_label, threshold)
        print(f'Threshold shape: {threshold.shape}, contour shape: {contour.shape}')

        # harta fractală
        fractal_map, fractal_map_max = calculate_local_fractal_dimensions(raw_image, box_size=8, step=1)

        fractal_map_tensor = torch.tensor(fractal_map, dtype=data[0].dtype)
        fractal_map_max_tensor = torch.tensor(fractal_map_max, dtype=data[0].dtype)

        data_with_fractal = torch.cat((data, fractal_map_tensor.unsqueeze(0).unsqueeze(-1), fractal_map_max_tensor.unsqueeze(0).unsqueeze(-1)),dim=0)
        output_path = file_path[:-3] + "_with_fractal_map.pt"
        ...
        axes[0].imshow(fractal_map_tensor, cmap='viridis', alpha=0.5)
        axes[1].imshow(fractal_map_max_tensor, cmap='viridis', alpha=0.5)

        plt.savefig(file_path[:-3] + "_debugging_fractal.png")
            

        torch.save(data_with_fractal, output_path)
